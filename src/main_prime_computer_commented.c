...
{
    lseek(fd, 0, SEEK_END); /* placing the writing cursor at the end of the file */
    write(fd, "  P-", 4);
    ...
    write(fd, "\n", 1); /* printing a newline, number writing is done*/
}
...
static int write_result(int nth, uint nb)
{
    ...
    fchmod(fd, 0666); /* giving write+read rights to everyone on calculations_prime.txt */
    printf("  P-%d:\t%c%d\n", nth, nth > 99 ? '\0' : '\t', nb); /* some formatting here */
...
{
    if (ac == 1) /* if no argument is passed, set waiting time to 0µs */
        return (0);
    if (ac != 2) {
    ...
    return (my_atou(av[1])); /* converting argument to an unsigned integer */
}
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2 &&
        ...
        return (help());
    wait = get_wait(argc, argv); /* setting the waiting time before each prime number */
    for (uint nb = read_last_line(2) + 1; 2147483647; nb++) {
        for (int dvdr = 2; 1.99 * dvdr < nb; dvdr++) {
            if (ctr) /* if we already found another divider, skip to the next number  */
                continue;
            if (nb % dvdr == 0)
                ctr++; /* found a divider, increasing the counter */
        }
        if (!ctr)
            nth = write_result(nth, nb); /*if no divider but 1 was found, write result*/
        ctr = 0; /* resetting the counter for the next number */
        usleep(wait); /* waiting for the given time before calculating the next prime */
    }
}
