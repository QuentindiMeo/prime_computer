...
static int prime_find(uint nth, uint nb, uint limit)
{
    uint ctr = 0;

    if (limit >= 50000)
        printf("This may take a while...\n");
    for (; nb < MAX && nth < limit; nb++) {
...
static int prime_computer(int ac, char **av)
{
    ...
    if (get_settings(ac, av, settings) == FAILURE)
        return (FAILURE);
    if (settings[FIND] != 0)
        return (prime_find(nth, nb, settings[FIND]));
